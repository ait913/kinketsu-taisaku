...
type MaterializeRuleRow = typeof recurringRule.$inferSelect;

async function insertGeneratedRecord(rule: MaterializeRuleRow, userId: string, yearMonth: string): Promise<void> {
...
export async function materializeRule(ruleId: number, userId: string, fromMonth: string, toMonth: string): Promise<{ created: number }> {
  ...
  for (const yearMonth of monthRange(start, endByRule)) {
    ...
    await insertGeneratedRecord(rule, userId, yearMonth);
    created += 1;
  }
  return { created };
}
...
export async function ensureMaterialized(userId: string): Promise<void> {
  ...
  const rules = await db.select().from(recurringRule).where(and(eq(recurringRule.userId, userId), eq(recurringRule.active, true)));
  for (const rule of rules) {
    const start = rule.startMonth > currentMonth ? rule.startMonth : currentMonth;
    const endByRule = rule.endMonth && rule.endMonth < toMonth ? rule.endMonth : toMonth;
    if (endByRule < start) continue;

    for (const yearMonth of monthRange(start, endByRule)) {
      const [existing] = await db.select({ id: record.id }).from(record).where(and(
        eq(record.sourceRuleId, rule.id),
        eq(record.yearMonth, yearMonth),
      )).limit(1);
